SNP_indel_isolator_S288c <- function(df, filename, polys) {
  ...
  df <-rename(df, c("pos"="pos_c"))
 
  merged_data <- merge(polys, df, by=c("chrom", "pos_c"), all=FALSE, sort=FALSE)
  ...
  write.table(merged_data,file=sprintf("%sOut_S288c.txt",filename), row.names=F, quote=F, sep='\t')
  ...
  dftel12 <- df[which(df$pos_c<59382),]
  dftel13 <- df[which(df$pos_c>51019),]
  dftel1 <- merge(dftel11, dftel12, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  dftel1 <- merge(dftel1, dftel13, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  
  dfmsh21 <- df[which(df$pos_c>147382),] 
  dfmsh22 <- df[which(df$pos_c<150276),] 
  dfmsh23 <- df[which(df$chrom==15),] 
  dfmsh2 <- merge(dfmsh21, dfmsh22, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  dfmsh2 <- merge(dfmsh2, dfmsh23, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  
  dfndt801 <- df[which(df$pos_c>356561),] 
  dfndt802 <- df[which(df$pos_c<358444),] 
  dfndt803 <- df[which(df$chrom==8),] 
  dfndt80 <- merge(dfndt801, dfndt802, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  dfndt80 <- merge(dfndt80, dfndt803, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  
  dfrad241 <- df[which(df$pos_c<538279),]
  dfrad242 <- df[which(df$pos_c>536300),]
  dfrad243 <- df[which(df$chrom==5),]
  dfrad24 <- merge(dfrad241, dfrad242, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  dfrad24 <- merge(dfrad24, dfrad243, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  
  dfsml11 <- df[which(df$pos_c>159383),] 
  dfsml12 <- df[which(df$pos_c<159697),] 
  dfsml13 <- df[which(df$chrom==13),] 
  dfsml1 <- merge(dfsml11, dfsml12, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  dfsml1 <- merge(dfsml1, dfsml13, by=c("chrom", "pos_c", "reads_all"), all=FALSE, sort=FALSE)
  ...
  write.table(gentable,file=sprintf("%sGenotype_S288c.txt",filename), row.names=F, quote=F, sep='\t')
